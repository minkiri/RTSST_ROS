...
def eval_videos(net, dataloaders):
    ...
    if len(dataloaders) == 0:
        logging.error("Error: No data loaded in eval_dataloaders. Please check your dataset paths.")
        return
    ...
    for idx in range(data_length):
        ...
        if cr is not None and dv is not None and ss is not None:
            crop_ratio.update(cr)
            distortion_value.update(dv)
            stability_score.update(ss)
        else:
            logging.warning(f"Skipping index {idx} due to missing metrics.")
